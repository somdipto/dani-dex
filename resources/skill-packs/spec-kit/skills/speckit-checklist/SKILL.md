...
description: "Spec-driven development (spec-kit): Generate a custom checklist for the current feature based on user requirements."
...
3. **Clarify intent (dynamic)**: Derive up to THREE initial contextual clarifying questions (no pre-baked catalog). They MUST:
   ...
   - Be skipped individually if already unambiguous in `(the user's request in this conversation)`
...
4. **Understand user request**: Combine `(the user's request in this conversation)` + clarifying answers:
