...
import { decodeTeamProtocolV1HttpResponse, type TeamProtocolV1JsonObject } from "./v1";
...
export function decodeProfileV1Draft(value: unknown, complete: boolean): TeamProtocolV1JsonObject {
  if (
    ...
    throw new Error("Invalid profile v1 draft.");
  return {
    name: value.name,
    title: value.title,
...
export function decodeProfileV1Request(generate: boolean, value: unknown): TeamProtocolV1JsonObject {
  if (!isDynamicRecord(value) || (value.agentId !== undefined && !identifier(value.agentId)))
    throw new Error("Invalid profile v1 request.");
  const agent: TeamProtocolV1JsonObject = value.agentId === undefined ? {} : { agentId: value.agentId };
  if (generate) {
    if (!bounded(value.prompt, 100_000) || !value.prompt.trim()) throw new Error("Invalid profile v1 prompt.");
    return {
      prompt: value.prompt,
      ...agent,
      ...(value.draft === undefined ? {} : { draft: decodeProfileV1Draft(value.draft, false) }),
    };
  }
  if (
    ...
    throw new Error("Invalid profile v1 save.");
  return {
    operationId: value.operationId,
    ...agent,
    draft: decodeProfileV1Draft(value.draft, true),
    ...(value.initialMessage === undefined ? {} : { initialMessage: value.initialMessage }),
  };
}

export function decodeProfileV1Response(generate: boolean, value: unknown): TeamProtocolV1JsonObject {
  if (generate) return decodeProfileV1Draft(value, true);
  if (!isDynamicRecord(value)) throw new Error("Invalid profile v1 response.");
  // Agent and sidebar fields retain their already frozen v1 representations and projections.
  return {
    agent: decodeTeamProtocolV1HttpResponse("PATCH", "/v1/agents/profile-saved", 200, value.agent),
    layout: decodeTeamProtocolV1HttpResponse("GET", "/v1/sidebar-layout", 200, value.layout),
  };
}
