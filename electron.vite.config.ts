...
const rendererPort = readRendererPort(process.env.DANI_DEX_DEV_RENDERER_PORT);
...
function readRendererPort(value: string | undefined): number | undefined {
  ...
  if (!Number.isInteger(port) || port < 1_024 || port > 65_535) {
    throw new Error("DANI_DEX_DEV_RENDERER_PORT must be an integer from 1024 to 65535.");
  }
  return port;
}
