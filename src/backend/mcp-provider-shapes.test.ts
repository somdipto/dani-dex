...
describe("needsManagedRuntime", () => {
  ...
  it("skips the wait once the managed store names the command", async () => {
    ...
    const tools = { binDirectories: [], commandAliases: { "dani-dex-no-such-command": "/managed/bin/tool" } };
...
describe("resolveMcpCommand", () => {
  ...
  it.runIf(process.platform !== "win32")("resolves a command once and reuses the answer", async () => {
    const directory = await mkdtemp(join(tmpdir(), "dani-dex-mcp-command-"));
    const executable = join(directory, "dani-dex-fake-server");
    try {
      await writeFile(executable, "#!/bin/sh\nexit 0\n");
      await chmod(executable, 0o755);
      expect(await resolveMcpCommand("dani-dex-fake-server", directory)).toBe(executable);
      ...
      clearMcpCommandCache();
      expect(await resolveMcpCommand("dani-dex-fake-server", directory)).toBeNull();
    } finally {
  ...
  it.runIf(process.platform !== "win32")("does not remember that a command was missing", async () => {
    const directory = await mkdtemp(join(tmpdir(), "dani-dex-mcp-command-"));
    const executable = join(directory, "dani-dex-late-server");
    try {
      expect(await resolveMcpCommand("dani-dex-late-server", directory)).toBeNull();

      // Installing the missing tool has to be enough. Remembering the miss would make a restart
      // the only way to be believed.
      await writeFile(executable, "#!/bin/sh\nexit 0\n");
      await chmod(executable, 0o755);
      expect(await resolveMcpCommand("dani-dex-late-server", directory)).toBe(executable);
    } finally {
      await rm(directory, { recursive: true, force: true });
    }
  });
...
describe("usableMcpServer with a managed tool runtime", () => {
  ...
  it.runIf(process.platform !== "win32")("lends a program only where the machine has none", async () => {
    const own = await mkdtemp(join(tmpdir(), "dani-dex-mcp-own-"));
    const managed = await mkdtemp(join(tmpdir(), "dani-dex-mcp-managed-"));
    try {
      for (const directory of [own, managed]) {
        await writeFile(join(directory, "dani-dex-fake-npx"), "#!/bin/sh\nexit 0\n");
        await chmod(join(directory, "dani-dex-fake-npx"), 0o755);
      }
      const tools = {
        binDirectories: [managed],
        commandAliases: { "dani-dex-absent-npx": join(managed, "bunx") },
      };
      ...
      const installed = await usableMcpServer(stdio("dani-dex-fake-npx"), tools);
      expect(installed.command).toBe(join(own, "dani-dex-fake-npx"));
      ...
      const lent = await usableMcpServer(stdio("dani-dex-absent-npx"), tools);
