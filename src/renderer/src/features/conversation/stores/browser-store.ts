import type { BrowserControlSession, BrowserPreview, BrowserTab } from "@dani-dex/contracts/ipc";
...
export function createBrowserStore(deps: BrowserStoreDeps) {
  ...
  async function openBrowserAddress(address = deps.browserAddress(), newTab = false) {
    ...
    if (currentTab) {
      ...
      try {
        await window.danidex.browser.navigate({ tabId: currentTab.id, url });
      } catch {
    ...
    const request = (async () => {
      try {
        const tab = await window.danidex.browser.open({
          url,
  ...
  async function reloadBrowserTab(tabId: string) {
    ...
    const analytics = desktopAnalytics.scope();
    try {
      await window.danidex.browser.reload(tabId);
  ...
  async function navigateBrowserTab(tabId: string, direction: "back" | "forward") {
    ...
    const target = agentId ? { agentId, serverId: deps.props.server?.id ?? "local" } : undefined;
    try {
      await window.danidex.browser.navigate({ tabId, direction });
    } catch {
